...
- (IBAction)showAssetTypePicker:(UIButton *)sender
{
    ...
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController: assetTypePicker];
        self.assetPopover = [[UIPopoverController alloc] initWithContentViewController: navController];
        self.assetPopover.delegate = self;
        [self.assetPopover presentPopoverFromRect: self.assetTypeButton.frame
                                           inView: self.view
                         permittedArrowDirections: UIPopoverArrowDirectionAny
                                         animated: YES];
    }
